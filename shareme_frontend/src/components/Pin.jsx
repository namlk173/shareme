...
const Pin = ({ pin: { postedBy, image, _id, destination, save } }) => {
  ...
  const [saved, setSaved] = useState(
    !!save?.filter((item) => item.postedBy._id === user?.sub)?.length
  );
    ...
    <div className="m-2">
        ...
        {postHovered && user && (
          ...
          >
            <div className="flex items-center justify-between">
              ...
              {saved ? (
                <button className="bg-red-500 text-white font-bold px-5 py-1 text-base rounded-3xl outline-none cursor-auto">
                  {save?.length} Saved
                </button>
              ) : (
                <button
      ...
      >
        <img
          ...
          alt="user-profile"
        />
        <p className="font-semibold capitalize text-base md:text-sm">{postedBy?.userName}</p>
      </Link>
    </div>
  );
};
